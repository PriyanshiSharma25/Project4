...
public class UserModel {
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sb = new StringBuffer("SELECT * FROM ST_USER WHERE 3=3");
		...
		if (bean != null) {
			...
			if (bean.getMobileNo() != null && bean.getMobileNo().length() > 0) {
				sb.append(" AND MOBILE_NO= " + bean.getMobileNo());
			}
			if (bean.getRoleId() > 0) {
			...
			if (bean.getUnSuccessfullLogin() > 0) {
				sb.append(" AND UNSUCCESSFULL_LOGIN=" + bean.getUnSuccessfullLogin());
			}
			if (bean.getLastLogin() != null && bean.getLastLogin().getTime() > 0) {
				sb.append(" AND LAST_LOGIN=" + bean.getLastLogin());
			}
			if (bean.getRegisteredIP() != null && bean.getRegisteredIP().length() > 0) {
				sb.append(" AND REGISTERED_IP LIKE '" + bean.getRegisteredIP() + "%'");
			}
			if (bean.getLastloginIP() != null && bean.getLastloginIP().length() > 0) {
				sb.append(" AND LOGIN_IP LIKE '" + bean.getLastloginIP() + "%'");
			}
		}
